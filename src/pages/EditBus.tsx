...
const EditBus: React.FC<BusProps> = ({ id }) => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const busId = id || window.location.hash.replace('#editbus/', '');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      console.log('Otobüs güncelleniyor:', formData);
      setIsSubmitted(true);
      
      // Form başarıyla gönderildikten sonra bildirim göster
      setTimeout(() => {
        setIsSubmitted(false);
        window.history.back(); // Otobüsler sayfasına geri dön
      }, 2000);
    } else {
      setErrors(newErrors);
    }
  ...
  return (
    <div className="edit-bus-container">
      <div className="page-header">
        <h1>Otobüs Düzenle</h1>
        <p>Otobüs bilgilerini güncellemek için aşağıdaki formu kullanın.</p>
      </div>

      {isSubmitted && (
        <div className="success-message">
          <p>✓ Otobüs bilgileri başarıyla güncellendi!</p>
        </div>
      )}
